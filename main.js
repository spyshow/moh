...
ipc.on('updateDatabase', function (event, project_ID, project_title) {
  var file = app.getPath('desktop') + '\\' + 'Update_Database-' + project_title + '-' + getDate() + '.pdf';
  var connection1 = new sql.Connection(config, function (err) {
    if (err) {
      ...
      request
        ...
        .query('SELECT [issues].[id],[issues].[charm],[issues].[status],[issues].[vsn]' +
          ' FROM [issues] ' +
  ...
  submenu: [{
    ...
    click() {
      mainWindow.webContents.executeJavaScript(`
          ...
          var project_title = project_name.options[project_name.selectedIndex].text;
          var project_ID = project_name.options[project_name.selectedIndex].value;
          ipcRenderer.send('updateTFS',project_ID,project_title);
        `);
    }
  }, {
    type: 'separator'
